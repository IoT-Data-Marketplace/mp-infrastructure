...
resource "aws_autoscaling_group" "eks_worker_asg" {
  ...
  name     = format("%s-%s-asg-%s", var.cluster_name, var.worker_group_name, each.key)
  ...
  dynamic "launch_template" {
    for_each = var.instance_lifecycle == "spot" ? [] : [
    "on-demand"]
    content {
  ...
  tag {
    key                 = "Name"
    value               = format("%s-%s-asg-%s", var.cluster_name, var.worker_group_name, each.key)
    propagate_at_launch = true
  }
