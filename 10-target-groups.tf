resource "aws_alb_target_group" "mp_web_client_target_group" {
  ...
  port     = local.ingress_config.iot_mp_web_client_node_port
  protocol = "HTTP"
...
resource "aws_alb_target_group" "mp_api_gateway_target_group" {
  ...
  port     = local.ingress_config.iot_mp_api_gateway_node_port
  protocol = "HTTP"
  vpc_id   = module.vpc.vpc_id
...
resource "aws_alb_target_group" "istio_ingress_gateway_tg" {
  ...
  health_check {
    ...
    protocol            = "HTTP"
    path                = "/healthz/ready"
    port                = local.ingress_config.istio_ingress_gateway_health_status_node_port
    healthy_threshold   = 2
    unhealthy_threshold = 2
    timeout             = 2
  }
